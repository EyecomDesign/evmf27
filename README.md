...
This project includes a workflow to deploy the built assets to GitHub Pages, so you can import the script in your site without running a local server.

### Setup

1. Initialize git and push this repo to GitHub (if not already):
   ```bash
   git init
   git add .
...
### Using the script in your site

After deployment, your script will be available at:

```
https://<username>.github.io/<repo-name>/index.js
```

Replace `<username>` with your GitHub username (or org name) and `<repo-name>` with the repository name.

**Example:** For a repo `evmf27` under user `justingfrerer`:

```html
<script defer src="https://justingfrerer.github.io/evmf27/index.js"></script>
```
